...
# Note: We are only getting the coordinates of all genes that intersect with LADs (ignore LFC2 in the file name)
...
make_count_df <- function(sample_name) {
  ...
  degs_in_lads <- read_tsv(
    paste(in_dir, sample_name, "/", sample_name, deg_lad_file_suffix, sep = ""),
    col_names = c(
  ...
  count_df <- merged_df |>
    count(
      lamin, lad_status, regulation_level,
      name = "num_genes"
    ) |>
    mutate(
